"""
slight alterations were made to the original simulator to enable the analysis of link failures
"""
...
class Network(object):
    ...
    def __init__(self, G, network_type):
        ...
        if isinstance(G, Network):
            ...
            self._all_shortest_paths = dict(nx.all_pairs_shortest_path(self._network)) ###dictionay mit Liste Paare + ihre kürzesten Wege?
...
class ZeroDetourBus(object):
    ...
    """
    """
    Alteration to the original code: in the original generator, only the network, the request generator and the network type were passed.
    Here it is possible to pass the parameters of a previous simulation. This enables the simulation of a failure durig operating time.
    By default the parameters are set so that the bus starts empty.
    """
    def __init__(self, network, req_gen, network_type, initpos=None,time=0,remaining_time=0,next_stop=None,stoplist=[],req_data=dict(),insertion_data=[]):
        self.network_type = network_type
        ...
        self.stoplist:List[Stop]=stoplist
    ...
    def fast_forward(self, t):
        ...
        if t < self.time:
            ...
            # There can't be any need to process stops.   ###what means middle of an edge? Why do we not need to process stops here?
